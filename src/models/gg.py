    ...
    def init_variables(self):
        ...
        for i in range(self.n_nodes):
            for j in range(self.n_nodes):
                self.g[i, j] = self.solver.IntVar(0, self.n_nodes, '')
    ...
    def init_constraints(self):
        ...
        for i in range(0, self.n_nodes):
            for j in range(1, self.n_nodes):
                if i != j:
                    self.solver.Add(0 <= self.g[i, j] <= (self.n_nodes - 1)*self.x[i, j])
            
            if i > 0:
                self.solver.Add(self.solver.Sum(self.g[j, i] for j in range(self.n_nodes)) - 
                    self.solver.Sum(self.g[i, j] for j in range(1, self.n_nodes)) == 1)
